...
所有布尔参数均使用 `argparse.BooleanOptionalAction`，所以既可写
`--refine`，也可写 `--no-refine`。同理可用 `--verbose/--no-verbose`、
`--save/--no-save` 等形式。
...
lr_x_effective = spectral_step_fraction /
                 (2 * (lambda_max(W) + dual_init_effective))
W = (Q + Q.T) / 2
```
...
| `--refine` | 布尔 | `False` | PDBO 结束后执行贪心 one-flip 局部搜索。MIS/MC 使用稀疏增量 QUBO gain；LABS 直接优化原始能量。 |
| `--refine_max_passes` | 整数或不填 | `None` | one-flip 最大扫描轮数；`None` 表示一直扫到完整一轮没有改进。只在启用 `refine` 时使用。建议传正整数。 |

推荐先启用 `--conditional_rounding --refine`。`rounding_samples` 会增加
`O(rounding_samples * batch * n)` 的随机样本存储与评分开销，大实例应从
较小值开始。精修在 PDBO 的 `timelimit` 之外执行；如需限制其工作量，应设置
`--refine_max_passes`。
...
实际 primal 步长和 dual 初值。条件舍入和精修启用时也会保存对应结果。MC/MIS
只有启用 `--refine` 时才会在文件中写出最终 bitstring；LABS 始终写出原序列
bitstring。
...
| 改善最终二进制解 | `--conditional_rounding --refine` | 增加结束阶段耗时，尤其无限轮精修。 |
...
python src/main.py \
  ...
  --conditional_rounding --refine --refine_max_passes 5 \
...
python src/main.py \
  --task mis --graph reg --n 10000 --d 3 --seed 0 \
  --conditional_rounding --refine --save
```
...
| `refine` | QUBO one-flip | QUBO one-flip | 原始 LABS energy one-flip |
...
- `--refine_max_passes` 当前未在 argparse 层校验；实际使用时应传正整数。
